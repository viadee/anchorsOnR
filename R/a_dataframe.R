...
explain.data.frame <- function(x, explainer, labels = NULL, n_labels = NULL,
  ...
  explanations = data.frame(matrix(ncol = 11, nrow = 0))
  colnames(explanations) = c("model_type", "case", "label", "label_prob", "feature", "feature_value",
                             "feature_weight", "feature_desc", "data","prediction", "precision")
  ...
  for (i in 1:nrow(x)){
    ...
    instancePrediction = performance(prediction, measures = list(acc)) #TODO: FIXME
    ...
    while(length(responseRaw) == 0){
      ...
      if (type == "eval_request"){
        ...
        newInstance = perturbate(explainer$perturbator, trainSet, trainSetDisc, instance, c(anchors, explainer$target))

        # Test pertubations with model
        pred = predict_model(explainer$model, explainer$preprocess(instance), ...) #, type = o_type
        measure = performance(pred, measures = list(acc)) #TODO: FIXME
    ...
    if ("anchorResult" %in% names(response)){
      ...
      instanceText = getInstanceText(instance[,-explainer$target])
      ...
      for(j in seq(1,length(featuresText))){
        ridx = 1 + nrow(explanations)
        explanations[ridx, "model_type"] = explainer$model$task.desc$type
        ...
        explanations[ridx, "feature"] = colnames(instance)[j]
        explanations[ridx, "feature_value"] = instance[,j]
