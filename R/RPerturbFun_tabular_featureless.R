...
perturbate.tabular.featureless <- function(perturbFun, dataset, bins, instance,
             anchors,
             probKeep,
             ...) {
    ...
    for (i in pertCols) {
      instance <- exchangeFeatureValue(dataset, instance, i, probKeep)
    }
    ...
    for (i in anchors) {
      if (is.null(bins[[i]]$doDiscretize) || bins[[i]]$doDiscretize) {
        ...
        instance <- exchangeFeatureValue(dataset[matchingRows, ], instance, i, probKeep)
      }
    }
...
perturbate.tabular.featurelessDisc <-
  function(perturbFun, dataset, bins, instance, anchors, probKeep, ...) {
    pertCols <- setdiff(seq(1, ncol(dataset), 1), anchors)
...
exchangeFeatureValue <- function(dataset, instance, feature, probKeep) {
